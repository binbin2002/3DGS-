...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None):
    ...
    # Create zero tensor. We will use it to make pytorch return gradients of the 2D (screen-space) means #创建一个全零张量。我们将使用它来使pytorch返回2D（屏幕空间）均值的梯度
    ...
    # Set up rasterization configuration 计算视场角的tan值，设置光栅化配置
    ...
    rasterizer = GaussianRasterizer(raster_settings=raster_settings) #创建一个 GaussianRasterizer 对象，用于渲染场景。
    ...
    if override_color is None:
        if pipe.convert_SHs_python: #如果希望在Python中从球谐函数中预计算颜色 SH跟颜色挂钩
